...
--
-- Servidor: 127.0.0.1
-- Tiempo de generación: 07-05-2023 a las 03:59:29
...
INSERT INTO `logs` (`id`, `accion`, `fecha`) VALUES
(1, 'Se actualizo el catalogo de materiales', '2023-05-06');
...
(0, 'Varillas de cobalto 60', 250, 10, '10.00'),
(1, 'Varillas de cobalto 60', 250, 10, '10.00'),
...
(2845, 'Varillas de cobalto 60', 250, 10, '10.00');

--
-- Disparadores `materiales`
--
DELIMITER $$
CREATE TRIGGER `nuevoMaterial` AFTER INSERT ON `materiales` FOR EACH ROW INSERT INTO logs (accion) VALUES('Se actualizo el catalogo de materiales')
$$
DELIMITER ;
...
--
-- Estructura Stand-in para la vista `materiales100`
-- (Véase abajo para la vista actual)
--
CREATE TABLE `materiales100` (
...
--
-- Estructura Stand-in para la vista `materiales1000`
-- (Véase abajo para la vista actual)
--
CREATE TABLE `materiales1000` (
`descripcion` varchar(40)
);
...
--
-- Estructura Stand-in para la vista `materialesanio2000`
-- (Véase abajo para la vista actual)
--
CREATE TABLE `materialesanio2000` (
`descripcion` varchar(40)
);
...
--
-- Estructura Stand-in para la vista `materialesmasvendidos`
-- (Véase abajo para la vista actual)
--
CREATE TABLE `materialesmasvendidos` (
`clave` int(11)
,`descripcion` varchar(40)
,`TotalEntregado` decimal(32,0)
);

-- --------------------------------------------------------

--
-- Estructura Stand-in para la vista `numero5000and5010`
-- (Véase abajo para la vista actual)
--
CREATE TABLE `numero5000and5010` (
`Clave` int(11)
,`RFC` varchar(15)
,`Numero` int(11)
,`Fecha` date
...
--
-- Estructura Stand-in para la vista `razonlikela`
-- (Véase abajo para la vista actual)
--
CREATE TABLE `razonlikela` (
`RFC` varchar(15)
,`Cantidad` int(11)
,`Fecha` date
,`Numero` int(11)
...
--
-- Estructura Stand-in para la vista `ventamateriales`
-- (Véase abajo para la vista actual)
--
CREATE TABLE `ventamateriales` (
`Numero` int(11)
,`Descripcion` varchar(40)
,`Cantidad` int(11)
,`Precio` float
...
--
-- Estructura para la vista `materiales100`
--
DROP TABLE IF EXISTS `materiales100`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `materiales100`  AS SELECT `materiales`.`descripcion` AS `descripcion` FROM (`materiales` join `entregan`) WHERE `materiales`.`clave` = `entregan`.`clave` AND `entregan`.`cantidad` > 100100  ;

-- --------------------------------------------------------

--
-- Estructura para la vista `materiales1000`
--
DROP TABLE IF EXISTS `materiales1000`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `materiales1000`  AS SELECT `materiales`.`descripcion` AS `descripcion` FROM (`materiales` join `entregan`) WHERE `materiales`.`clave` = `entregan`.`clave` AND `entregan`.`cantidad` > 10001000  ;

-- --------------------------------------------------------

--
-- Estructura para la vista `materialesanio2000`
--
DROP TABLE IF EXISTS `materialesanio2000`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `materialesanio2000`  AS SELECT `materiales`.`descripcion` AS `descripcion` FROM (`materiales` join `entregan`) WHERE `materiales`.`clave` = `entregan`.`clave` AND `entregan`.`fecha` between '2000-01-01' and '2001-01-01''2001-01-01'  ;

-- --------------------------------------------------------

--
-- Estructura para la vista `materialesmasvendidos`
--
DROP TABLE IF EXISTS `materialesmasvendidos`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `materialesmasvendidos`  AS SELECT `m`.`clave` AS `clave`, `m`.`descripcion` AS `descripcion`, sum(`e`.`cantidad`) AS `TotalEntregado` FROM (`materiales` `m` join `entregan` `e`) WHERE `m`.`clave` = `e`.`clave` AND `e`.`fecha` between '2001-01-01' and '2002-01-01' GROUP BY `m`.`clave`, `m`.`descripcion``descripcion`  ;

-- --------------------------------------------------------

--
-- Estructura para la vista `numero5000and5010`
--
DROP TABLE IF EXISTS `numero5000and5010`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `numero5000and5010`  AS SELECT `entregan`.`clave` AS `Clave`, `entregan`.`rfc` AS `RFC`, `entregan`.`numero` AS `Numero`, `entregan`.`fecha` AS `Fecha`, `entregan`.`cantidad` AS `Cantidad` FROM `entregan` WHERE `entregan`.`numero` between 5000 and 50105010  ;
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `proveedoreseducando`  AS SELECT `p`.`rfc` AS `rfc`, `p`.`razonsocial` AS `razonsocial` FROM ((`proveedores` `p` join `entregan` `e`) join `proyectos` `pr`) WHERE `p`.`rfc` = `e`.`rfc` AND `e`.`numero` = `pr`.`numero` AND `pr`.`denominacion` = 'Educando en Coahuila''Educando en Coahuila'  ;
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `proveedorestelevisa`  AS SELECT `p`.`rfc` AS `rfc`, `p`.`razonsocial` AS `razonsocial` FROM ((`proveedores` `p` join `entregan` `e`) join `proyectos` `pr`) WHERE `p`.`rfc` = `e`.`rfc` AND `e`.`numero` = `pr`.`numero` AND `pr`.`denominacion` = 'Televisa en accion''Televisa en accion'  ;

-- --------------------------------------------------------

--
-- Estructura para la vista `razonlikela`
--
DROP TABLE IF EXISTS `razonlikela`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `razonlikela`  AS SELECT `entregan`.`rfc` AS `RFC`, `entregan`.`cantidad` AS `Cantidad`, `entregan`.`fecha` AS `Fecha`, `entregan`.`numero` AS `Numero` FROM `entregan` WHERE `entregan`.`numero` between 5000 and 5010 AND `entregan`.`rfc` in (select `proveedores`.`rfc` from `proveedores` where `proveedores`.`razonsocial` like 'La%')  ;

-- --------------------------------------------------------

--
-- Estructura para la vista `ventamateriales`
--
DROP TABLE IF EXISTS `ventamateriales`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `ventamateriales`  AS SELECT `e`.`numero` AS `Numero`, `m`.`descripcion` AS `Descripcion`, `e`.`cantidad` AS `Cantidad`, `m`.`precio` AS `Precio`, `m`.`PorcentajeImpuesto` AS `PorcentajeImpuesto`, `e`.`cantidad`* `m`.`precio` * (1 + `m`.`PorcentajeImpuesto` / 100) AS `Importe` FROM (`entregan` `e` join `materiales` `m`) WHERE `e`.`clave` = `m`.`clave``clave`  ;
